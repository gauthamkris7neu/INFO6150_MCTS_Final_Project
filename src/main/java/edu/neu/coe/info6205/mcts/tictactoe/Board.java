    ...
    public void printBoard() {
        for (int i = 0; i < boardValues.length; i++) {
            for (int j = 0; j < boardValues[i].length; j++) {
                // Print 'X' for player 1, 'O' for player 2, and ' ' for empty
                if (boardValues[i][j] == P1) {
                    System.out.print("X ");
                } else if (boardValues[i][j] == P2) {
                    System.out.print("O ");
                } else {
                    System.out.print("- ");  // Assuming 0 is the value for empty
                }
            }
            System.out.println();  // New line at the end of each row
        }
        System.out.println("-------");  // To separate between board states
    }
